   ...
   $.get("data/publications/data", function(data) {
      ...
      $("conferences",data).each(function(){
         $('p',$(this)).each(function(){
            ...
            as = $('a',(this)); // 作者
			if ( as.length == 1 ) {
			   html_stack += convertMyName(as[0].text()) + ",";
			} else if ( as.length == 2 ) {
			   html_stack += convertMyName(as[0].text()) + " and " + convertMyName(as[1].text()) + ",";
			} else {
			   i = 0;
			   for(; i < as.length - 1; ++i) {
			      html_stack += convertMyName(as[i].text()) + ", ";
			   }
			   html_stack += "and " + convertMyName(as[i].text()) + ",";
			}
			html_stack += "<br>\n";
			...
			if ( d != "" ) {
			   ...
			   if ( nop == "" ){
			      html_stack += "[<a class=\"publications-paper\" href=\"data/publications/papers/" + getSurname(as[0].text()) + " " + y + " - " + t + " - author version.pdf\"></a>]";
			   }
